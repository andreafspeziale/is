...
H/v scaling is not the answer. A good profiling and thoughtful resource allocation is part of the game, but as a SW engineer the first thing you need to do is offload the heavy computations to a pool of [worker threads](https://nodejs.org/api/worker_threads.html), allowing the main thread to remain responsive for handling I/O, probes and other tasks.
...
export class WorkerPool implements OnModuleInit, OnModuleDestroy {
  ...
  // ... private methods: runTask, spawnWorker, releaseWorker, drainQueue, replaceWorker ...
...
The worker runs in its own V8 isolate, it can saturate a full CPU core without affecting the main thread's ability to serve probes, accept connections, or dispatch other I/O. The implementation is minimal, it receives the payload, runs normalization, scores similarities and posts back:
